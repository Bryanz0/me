...
function ToggleLang() {
  ...
  return (
    <div className="custom-select" id="custom-select">
      <button
        type="button"
        className={`select-trigger-${theme}`}
        id="select-trigger"
        onClick={() => showAndHideLangOptions(showLangOptions)}
        aria-expanded={showLangOptions}
      ...
      <ul
        ...
        id="optionsLanguage"
        style={{ display: showLangOptions === true ? "block" : "none" }}
      >
        {languages.map((lang) => (
          <li
            key={lang}
            ...
            onClick={() => {
              changeLang(lang);
              showAndHideLangOptions(showLangOptions);
